...
- **Not run on an actual device or emulator.** `flutter build apk --debug` now succeeds (see the Verification log below) — the whole app, including `camera`/`video_player`/`permission_handler` plugins, compiles correctly for Android — but the resulting APK has never been installed and launched, so `camera_record_view.dart` and the upload pipeline have never actually executed against real hardware.
...
**Still not verified**: the APK has not been installed/launched on an actual device or emulator — only compiled. iOS remains completely unverified (no Mac/Xcode here).
