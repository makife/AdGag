...
**A third, self-inflicted bug immediately followed**: the rebuild above was run as plain `flutter build apk --debug`, without `--dart-define-from-file=env/dev.json`. `EnvConfig.assertConfigured()` correctly throws when `SUPABASE_URL`/`SUPABASE_ANON_KEY` are empty, but it throws inside `main()` before `runApp()` — so Flutter never paints a first frame, and the app just shows Android's default white window background forever. The user reported exactly this ("opens to a white page, stays there") on installing that build. Confirmed by APK size — the config-less build was ~202MB vs. the correct ~166MB — and fixed by rebuilding with `flutter build apk --debug --dart-define-from-file=env/dev.json`, which reproduced the last known-good size exactly (174,225,502 bytes). `flutter build apk --debug` for this project is **never** valid without that flag: it doesn't fail the build, it silently ships a white-screen APK.
