...
- **Text/GIF overlay burned into the exported video is still not implemented** — the user explicitly asked for this ("font desteği gif desteği"), but it's out of scope for `easy_video_editor` (no compositing API) and needs a real licensing decision (ffmpeg-class tooling is typically GPL) before any implementation work starts. This is the top open item from the editor-scope-expansion round above — needs your call on which native toolchain/licensing tradeoff before building it, not another silent attempt.
