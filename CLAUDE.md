...
- **The advanced editor's timeline is functional but not visually polished** — zones/overlays are added via a dialog and shown as removable chips, not a draggable/resizable colored-track timeline widget you'd see in a "real" video editor. `VideoProject`/the filter-graph builder don't care how a zone was created, so this is a pure UI enhancement that can be added later without touching the model or export pipeline.
