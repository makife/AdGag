...
## Native editor polish batch: colored handles/playhead, tap-to-pause video, a music-segment race fix, and a defensive MergingMediaSource flag for the reported freeze (2026-09-26)

Direct follow-up feedback, before the ExoPlayer pivot below was re-confirmed on-device: colored trim/music drag handles, a colored playhead line, the music segment/handle not appearing until an unrelated trim drag "unstuck" it, freezes reported specifically when music is attached, and a request that tapping the video itself (not only the center button) toggle play/pause.

**Colored UI, `EditorTimeline.kt`**: both `TrimHandle`'s visual bar and the Music row's right-edge handle now use `AdGagColors.GradientPink` (was plain white) — matches this screen's existing "active/accent" convention (`EditorToolButton`'s active tint). The playhead line is now `AdGagColors.BrandGradient` (was plain white, width bumped 2dp→3dp for visibility) — the same 4-stop brand gradient already used sparingly elsewhere on this screen (the Next button), not a new one-off color.

**Music segment/handle not appearing until a trim drag "fixes" it — real, reasoned fix, not fully device-confirmed**: traced to `EditorViewModel.setMusic()` computing `clipDurationMs = (trimEndMs - trimStartMs)` — if music is attached before `trimEndMs` has ever been defaulted from the player's own learned duration (still its untouched `0`), this degenerates to `0`, and the resulting `musicPlayDurationMs = min(probedMs, 0) = 0` renders the segment at only its coerced minimum handle width — easy to miss, matching "gözükmüyor" (doesn't show). A later, unrelated `setTrim()` call re-clamps music placement using the by-then-real `trimEndMs`, which is why it "appears" only after a trim round-trip. Fixed: `setMusic()` now falls back to the player's own already-known `durationMs` whenever `trimEndMs` hasn't been established yet (`effectiveClipEnd = if (trimEndMs > trimStartMs) trimEndMs else durationMs`), closing the race at its source. **Honesty note**: this is the most plausible mechanism found by code reading, not confirmed against an actual reproduction in this environment — width-clamping logic means the segment technically was never literally zero-width even before this fix, so if the report persists after this change, the real cause may be something else (e.g. a `BoxWithConstraints` first-measurement quirk) and needs a fresh screenshot to pin down.

**"Freezes when music is added" — one defensible mitigation, not a confirmed fix**: `MergingMediaSource(videoOnlySource, musicSource)` now passes `adjustPeriodTimeOffsets = true` (confirmed real/public via the downloaded `media3-exoplayer-1.11.0` sources: "whether to adjust timestamps of the merged media sources to all start at the same time") — the video branch's sample timestamps come from a `ClippingMediaSource` over the original file (offset by `trimStartMs`), while the music branch's come from a freshly-clipped/gap-prefixed file starting at its own local zero; without this flag ExoPlayer wasn't told these two periods should be treated as co-starting. **Stated honestly: the exact mechanism behind the reported freeze was not conclusively identified from code alone** (several other candidates — renderer reconfiguration when the MediaSource graph changes shape from 1 audio+1 video to 1 filtered-video+1 merged-audio, `SilenceMediaSource`→real-audio period transitions — were considered and are architecturally plausible but not ruled in or out without device logs); this is the single most defensible, low-risk change available, not a guaranteed resolution.

**Tap video to pause/resume, `EditorScreen.kt`**: the outer video `Box` (declared before the center play-button sibling, so the button still wins taps within its own bounds when visible) now carries `pointerInput(Unit) { detectTapGestures(onTap = { viewModel.togglePlayPause() }) }` — tapping anywhere on the video, not only the center button, now toggles play/pause.

`flutter analyze`: 0 issues. `flutter test`: 60/60 (unaffected — Kotlin-only). Real build verified: `flutter build apk --debug --dart-define-from-file=env/dev.json` succeeded; kernel_blob check confirmed real config compiled in.

**Not yet verified on a physical device.** Next test should confirm: handles/playhead now read as colored, not white; whether the music segment/handle appears immediately on attach without needing a trim round-trip; whether freezes on music-attach are gone, reduced, or unchanged (if unchanged, the next step is capturing exactly when in the attach sequence the freeze happens — first frame after tapping "Add music," or only once playback resumes); and that tapping the video body now reliably pauses/resumes.

## CompositionPlayer construction crash confirmed device-specific (not battery); pivoted the live preview to plain ExoPlayer + MergingMediaSource/ClippingMediaSource (2026-09-26)

The round below shipped `player.repeatMode = REPEAT_MODE_ONE` and the real timeline, both untested on-device at the time. Real-device retest surfaced a NEW crash, at a different point than any previously fixed this session: the checkpoint log stopped dead right after "building CompositionPlayer," before any composition/video file was even involved — i.e. `CompositionPlayer.Builder(context).build()` itself was crashing, at construction time. First screenshot showed the device at 6% battery, a real, non-code confounding factor worth ruling out before trusting this as a code bug — so the crash site was split into two separate log lines (`Builder(context) constructed` vs. `build() returned`) and shipped for a charged-device retest (commit `2783fa4`). **The user retested fully charged — same crash, same point, Retry (which just re-attempts the identical construction) closes the app.** This conclusively rules out battery/power-saving as the cause: `CompositionPlayer.Builder(context).build()` is genuinely incompatible with something about this specific device (most likely its GPU/driver/decoder stack), not an environmental fluke.

**Read `CompositionPlayer.java`'s own decompiled source (media3-transformer-1.11.0-sources.jar) to understand exactly why this construction path is comparatively risky**: its private constructor starts a dedicated `HandlerThread` at `THREAD_PRIORITY_AUDIO` and sets up a GL-backed video graph pipeline (`playbackVideoGraphWrapper`) immediately, inline in `build()` — genuinely heavy, native/GPU-touching work happening before any Surface is even attached or playback started. `CompositionPlayer` is also still `@UnstableApi` in Media3 itself as of 1.11.0 — a newer, actively-evolving class built specifically for the Composition-preview use case, not a decade-proven legacy path.

**Pivot: replaced the live preview player with plain `ExoPlayer` + `MergingMediaSource`/`ClippingMediaSource`/`FilteringMediaSource`** (all verified against the real downloaded `media3-exoplayer-1.11.0-sources.jar` before use, not guessed — this project's standing verification discipline). Honesty check done before committing to this: grepping the real sources confirmed `MergingMediaSource`, `ClippingMediaSource`, and `ProgressiveMediaSource` are ALSO marked `@UnstableApi` — but this must not be read as "equally risky as CompositionPlayer." Media3 applies `@UnstableApi` broadly, as a matter of policy, across almost the entire `media3-exoplayer` module below the top-level `Player` interface — it signals "API surface may still change between releases," not "unreliable at runtime." These three classes are part of ExoPlayer's decade-old, extremely widely-deployed core architecture (the standard, textbook way to combine a video-only source with a separately-sourced audio track into one Player/one clock), structurally unlike `CompositionPlayer`'s unusually heavy construction-time native/GPU setup. `ExoPlayer.Builder(context).build()` itself does none of that heavy work at construction time — real rendering setup is deferred until a Surface is attached and playback actually starts, making it a lighter, safer construction path by design, independent of the `@UnstableApi` label either class carries.

**What changed, `EditorViewModel.kt`**:
- `val player: CompositionPlayer` → `val player: ExoPlayer`, built via `ExoPlayer.Builder(context).build()`; `repeatMode = REPEAT_MODE_ONE` carried over unchanged.
- New `buildPreviewMediaSource()` replaces `player.setComposition(buildComposition())` for the PREVIEW path only: video clipped via `ClippingMediaSource` (constructor confirmed: `(mediaSource, startPositionUs, endPositionUs)`); when music is attached, the clip's own audio is deterministically excluded via `FilteringMediaSource(videoSource, C.TRACK_TYPE_VIDEO)` (confirmed real/public — "publishes tracks of one type" — rather than relying on `DefaultTrackSelector`'s undocumented, non-deterministic tie-break between two simultaneous audio `TrackGroup`s from a naive merge), the music itself clipped the same way, `SilenceMediaSource(startOffsetUs)` + `ConcatenatingMediaSource` reproduce the same "start N ms into the clip" gap `Composition`'s `addGap` used to give, and `MergingMediaSource(videoOnlySource, musicSource)` combines them into one Player/one clock.
- **Real, honestly-scoped preview limitation**: plain ExoPlayer has no equivalent of `Composition`'s genuine N-way audio MIXING (that's exclusively `CompositionPlayer`/`Transformer` machinery). So when music is attached, the live preview plays ONLY the music — not the original clip's own audio blended with it — documented explicitly in the class's own doc comment as a preview-only simplification, not silently glossed over. **The exported Ad is completely unaffected**: `export()`/`buildComposition()` still goes through `Transformer`, entirely untouched by this pivot (it never used `CompositionPlayer` and was never implicated in the crash), and still correctly mixes the original audio (when not muted) together with music, sample-accurately, in the final published file.
- Mute (no music case): `player.volume = 0f/1f` — simpler and more reliable for a single already-selected audio track than rebuilding the MediaSource graph.
- Rotation: `rotateNinety()` no longer triggers a MediaSource rebuild at all (rotation was never part of ExoPlayer's playback graph to begin with — Effects/Transformations are CompositionPlayer/Transformer-only). `EditorScreen.kt`'s `PlayerSurface` now takes `Modifier.rotate(viewModel.rotationDegrees.toFloat())` — a Compose-level VISUAL approximation, not a re-decoded rotated frame (won't swap the surface's reported width/height at 90°/270°, unlike the real pixel rotation baked in at export time via the unchanged `ScaleAndRotateTransformation` in `buildComposition()`). Flagged honestly as a real, acceptable-for-now simplification given the alternative was a crashing player.
- `buildComposition()` (used only by `export()` now) is otherwise byte-for-byte unchanged — still builds the real `Composition` via `EditedMediaItemSequence`/`EditedMediaItem`, still the one source of truth for the published Ad's actual audio mix and rotation.

Every new class used (`FilteringMediaSource`, `SilenceMediaSource`, `ConcatenatingMediaSource`, `MergingMediaSource`, `ClippingMediaSource`, `DefaultDataSource.Factory`) had its exact constructor/method signature confirmed against real downloaded sources/class files before use — `DefaultDataSource.Factory(Context)`'s single-arg constructor was confirmed via the compiled `media3-datasource-1.11.0-api.jar`'s constant pool (`(Landroid/content/Context;)V`) since no sources jar for that module was already on hand. `PlayerSurface(player, modifier, surfaceType)`'s real signature (confirmed via the already-downloaded `media3-ui-compose-sources.jar`) takes `player: Player?`, so `ExoPlayer` (which implements `Player`, same as `CompositionPlayer` did) needed zero changes anywhere in `EditorTimeline.kt`/`EditorScreen.kt`'s existing `viewModel.player.seekTo/currentPosition/isPlaying` usage.

`flutter analyze`: 0 issues. `flutter test`: 60/60 (unaffected — Kotlin-only change). Real build verified: `flutter build apk --debug --dart-define-from-file=env/dev.json` succeeded; kernel_blob check confirmed real config compiled in.

**Not yet verified on a physical device — this is the single highest-priority thing to confirm next**, ahead of any further feature work: does `ExoPlayer.Builder(context).build()` avoid the crash entirely (the core hypothesis this whole pivot rests on); does playback/loop/trim/scrub work as before; does music preview audibly play (even though, per the documented limitation above, alone rather than mixed with original audio); does the mute toggle still work correctly with no music attached; does the rotation preview look acceptable even as a visual-only approximation; and — the actual bar for calling this resolved — does the editor reliably open at all now, on the exact device that crashed twice before.
